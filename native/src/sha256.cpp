...
public:
    void update(const unsigned char* data, std::size_t size) {
        ...
        while (size != 0) {
            ...
            std::copy_n(data, amount, block_.begin() + static_cast<std::ptrdiff_t>(used_));
            used_ += amount; data += amount; size -= amount;
            if (used_ == block_.size()) { transform(); used_ = 0; }
        }
    }

    std::string finish() {
        const auto bit_count = total_ * 8U;
        block_[used_++] = 0x80U;
        if (used_ > 56U) { std::fill(block_.begin() + static_cast<std::ptrdiff_t>(used_), block_.end(), 0); transform(); used_ = 0; }
        std::fill(block_.begin() + static_cast<std::ptrdiff_t>(used_), block_.begin() + 56, 0);
        for (unsigned index = 0; index < 8; ++index)
            block_[63U - index] = static_cast<unsigned char>(bit_count >> (index * 8U));
        transform();
        std::ostringstream out;
        for (const auto word : state_) out << std::hex << std::setw(8) << std::setfill('0') << word;
        return out.str();
    }
...
private:
    void transform() {
        ...
        for (std::size_t index=0; index<8; ++index) state_[index]+=work[index];
    }
...
std::string sha256_file(const std::filesystem::path& path) {
    ...
    if (!input) throw std::runtime_error("cannot hash output");
    Sha256 hash;
    ...
    if (!input.eof()) throw std::runtime_error("cannot read output for hashing");
    return hash.finish();
}
