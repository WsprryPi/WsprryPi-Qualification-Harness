...
def _validate_observations(
    ...
    if analyzer["version"] == "9" and plan["mode"] != "tone":
        ...
        if observations["analysis_outcome"] == "passed" and {
            q["event_index"] for q in quiet_windows
        } != {e["index"] for e in expected["events"] if e["rf_state"] == "off"}:
            _fail("passing observations lack complete quiet significance evidence")
        for quiet in quiet_windows:
            duration = float(capture["sample_count"]) / float(capture["sample_rate_hz"])
            ...
            if recomputed != quiet:
                _fail("quiet significance evidence contradicts its retained events or policy")
